...
4. If the `.license` file is missing, not valid JSON, expired, or the tier doesn't cover the requested domain:
   ...
   - For [PRO] or [ENTERPRISE] domains: respond with:
     *"You have paid skill files installed but no active license. Run `./setup.sh activate` with the email you purchased with and your license ID (from your welcome email). Don't have a license? Get one at https://www.hlarchitect.ai/#pricing"*
